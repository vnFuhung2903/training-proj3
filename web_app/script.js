...
var contractAddress = "0xCf7Ed3AccA5a467e9e704C703E8D87F634fB0Fc9"; // FIXME: fill this in with your contract's address/hash
...
async function getNeighbors(node) {
	const arr = await BlockchainSplitwise.getCreditor(node);
	let res = [];
	for(let i = 0; i < arr.length; ++i) {
		const owed = await BlockchainSplitwise.lookup(node, arr[i]);
		if(owed > 0) res.push(arr[i]);
	}
	return res;
}
...
async function add_IOU(creditor, amount) {
	...
	if(path !== null) {
		...
		for(let i = 0; i < path.length - 1; ++i) {
			const debt = await BlockchainSplitwise.lookup(path[i], path[i + 1])
			if(debt > minDebt) await BlockchainSplitwise.connect(paht[i]).add_IOU(path[i + 1], debt - minDebt);
		}
	}
